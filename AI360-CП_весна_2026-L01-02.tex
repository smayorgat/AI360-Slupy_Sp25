...
называется мера на \(\mathcal{Y}\), обозначаемая 
...
Для этого вводится понятие \emph{\textbf{цилиндра}
...
функцию \(P\) на цилиндрах ровно так, 
как это «прописано» соответствующей 
мерой из семейства \(\mathcal{Q}\)
(то есть, 
цилиндру \(C=C_{t_1,\ldots,t_n}(B_1\times\cdots \times B_n)\)
дать значение \(P(C) = P_{t_1,\ldots,t_n}(B_1\times B_n)\))
получается конечно-аддитивная функция
на алгебре всех цилиндров, и далее
доказательство занимается расширением 
этой конечно-аддитивной функции 
...
\begin{thm}\label{thm:kolmogorovcontver} (Колмогорова о непрерывной 
  версии процесса)
Пусть \(T=[0,\infty)\), \((\xi_t)_{t\in T}\) --- случайный процесс. Если
...
\begin{align*}
  ...
  \frac{|\xi_t(\omega)-\eta_t(\omega)|}{|t-s|^{\gamma}} 
  \leq \delta
  \big\}  \big) = 1
\end{align*}
